...
function intelExtractExports(filePath) {
  ...
  // Set instead of array eliminates the repeated `.includes()` dedupe checks
  // scattered through the CJS and ESM extraction loops. Insertion order is
  // preserved (Sets iterate in insertion order), so callers that depend on
  // declaration ordering still see the same sequence after `[...exports]`. (#313)
