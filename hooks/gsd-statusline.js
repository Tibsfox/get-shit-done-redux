...
function runStatusline() {
  ...
  try {
    ...
    if (session && fs.existsSync(todosDir)) {
      try {
        // Single-pass max-by-mtime scan avoids the temporary array allocation
        // and O(n log n) sort of the prior `.filter().map(statSync).sort()`
        // chain. Same syscall count; the sort and intermediate arrays are
        // unnecessary when we only need the newest match. (#305)
        let latest = null;
